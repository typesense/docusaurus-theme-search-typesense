...
function SearchPageContent() {
    ...
    const { typesense: { typesenseCollectionName, typesenseServerConfig, typesenseSearchParameters, externalUrlRegex, }, } = themeConfig;
    ...
    const algoliaHelper = algoliaSearchHelper(typesenseInstantSearchAdapter.searchClient, typesenseCollectionName, searchParams);
      ...
      <div className="container margin-vert--lg">
        ...
        {searchResultState.items.length > 0 ? (<main>
            {searchResultState.items.map(({ title, url, summary, breadcrumbs }, i) => (<article key={i} className={styles.searchResultItem}>
                  ...
                  {breadcrumbs.length > 0 && (<nav aria-label={translate({
